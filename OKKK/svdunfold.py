...
class SVDunfold:
    ...
    def __init__(self, x_ini, b, A, B, xmin,xmax,bmin,bmax,eps):
        ...
        self._nx=len(self._x_ini)
        self._nb=len(self._b)
        self._C=self.__second_deriv_matrix(eps)
        self._C_inv=np.linalg.inv(self._C)
        #assert(self._A.shape == self._nb), "Wrong dimensions of A: bins in b != rows in A"
    ...
    def unfolding(self,k):
        ...
        self._x_unfolded = w_tau * self._x_ini
        self._X_unfolded_covariance = W_tau * (np.outer(self._x_ini, self._x_ini))
    ...
    def __get_X_inv(self,A_tilde):
        ...
        for j in range(self._nx):
            for h in range(self._nx):
                for i in range(self._nb):
                    X_inv[j, h] += (A_tilde[i, j] * A_tilde[i, h]) / (self._x_ini[j] * self._x_ini[h])
        return X_inv
